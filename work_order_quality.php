...
<table class="table table-striped table-bordered " id="RecievedContainerTable">
	<thead>
		...
		where master_wo_status = 1 order by master_wo_id desc");
		...
		if(is_array($getRecieved)){
			foreach($getRecieved as $Recieved){
                ...
                <tr>
                    ...
                    <button class="ReturnFromQ btn btn-danger mt-1" data-id="<?php echo ($Recieved['master_wo_ref']); ?>">Return</button>
                    </td>
                </tr>
                <?php
			}
		...
		if(is_array($getPublished)){
			foreach($getPublished as $Published){
                ...
                <tr>
                	...
                	<td><?php echo $Published['mwoid_desc2'] ?></td>
                	<td>
                    <a href="work_order_main_edit?id=<?php echo $Published['master_wo_ref'] ?>" target="_blank">
                    <button class="btn btn-warning mt-1">View/Edit</button>
                    </a>
                    <button class="rePublish btn btn-success mt-1" data-id="<?php echo ($Published['master_wo_ref']); ?>">Re-Publish</button>
                    </td>
                </tr>
                <?php
			}
            ...
            <div class="row">
                <div class="col-12 ">
                                <div class="card card-warning">
                                  ...
                                  <div class="card-body text-justify">
<table class="table table-striped table-bordered " id="SentBackContainerTable">
	<thead>
    	<tr>
            <th>WO#</th>
            <th>Client</th>
...
<div class="row">
                <div class="col-12 ">
                                <div class="card card-danger">
                                  <div class="card-header">
                                    <h4>Marked Complete</h4>
                                  </div>
                                  <div class="card-body text-justify">
<table class="table table-striped table-bordered " id="CompleteContainerTable">
	<thead>
    	<tr>
            <th>WO#</th>
            <th>Client</th>
...
$("#SentBackContainerTable").DataTable();
$("#ReturnedContainerTable").DataTable();
$("#CompleteContainerTable").DataTable();
	...
	$('.publishQtoFM').click(function(e) {
		...
		bootbox.confirm("Are you sure you want to publish WO Number " + dataId + " ?", function(result){ 
			if(result){
				...
				{
					qualityToFactoryManager: dataId,
				},
...
$(document).ready(function(e) {
	$('.ReturnFromQ').click(function(e) {
		var dataId = ($(this).data("id"));

		bootbox.confirm("Are you sure you want to Send WO Number " + dataId + " back to Sales?", function(result){ 
			if(result){
